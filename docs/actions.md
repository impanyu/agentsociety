...
### 3.5 开局自省(goal bootstrap)
适用场景:agent 的目标栈为空时,`Kernel._build_agent_view` 会在 view 里附加一个
`goal_hint` 字段(见 `society/kernel.py` 的 `_GOAL_HINT_ZH`/`_GOAL_HINT_EN`,
按场景 `language` 选择中/英文提示)——典型触发场景是"历史沉淀"模式
(`society.history_extract`)组装出的后传场景:书中存活到终局的角色以
`goals: []` 起步,没有预设目标,需要自己想清楚接下来做什么;`archived: true`
的已故角色永不参与调度,不会看到这个提示。流程与
`society/skills/actions_skill_{zh,en}.md` 的"开局自省"一节完全一致:
1. `recall` 关于自己的过去(例如按名字/经历关键词查询),回忆自己是谁、经历
   过什么——这些记忆正是沉淀阶段写入共享 LTM 的原子条目;
2. `observe` 当前所在的 environment,了解现在身处何地、周围有谁、发生了什么;
3. `conclude` 把"我是谁 + 我现在的处境"综合成一句阶段性判断,写入 FIFO(尚不
   写入共享 LTM,见 §3.4 记忆卫生);
4. `push_goal` 设立一个根本目标(fundamental,压在目标栈栈底,基于第 3 步的
   判断);
5. `push_goal` 再设立一个当前的小目标(具体到眼下这一步该做什么)——此后目标
   栈不再为空,`goal_hint` 也就不再出现在 view 里,agent 转入其他 pipeline
   (消息处理/社交/移动等)正常推进。
