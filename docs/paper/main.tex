...
\newcommand{\sysname}{\textsc{Agentsensus}}
...
together: the oath in the peach garden (\begin{CJK}{UTF8}{gbsn}桃园结义\end{CJK})
is one fact with three owners, not three private paraphrases. \sysname{} builds
this observation into the storage layer itself.
\footnote{Simulations run natively in the source language; we quote the English
translation in text with the source in parentheses at first mention, and give
side-by-side Chinese/English originals in Appendix~\ref{app:examples}.}
...
\begin{enumerate}
  ...
  \item \textbf{Evidence.} Across four diverse scenarios and three strong
...
War and Peace (Vol.~I) & en & $\sim$30 & Books~1--2 & Book~3 \\
Russia--Ukraine timeline & en & $\sim$25 & 2022/02--2024/04 & 2024/05--2026/07 \\
\bottomrule
\end{tabular}
...
\paragraph{Models.} All agent brains and the sedimentation extractor use a
single fixed chat model (DeepSeek~V4~Pro) across \emph{every} system, so the
memory-layer comparison is held strictly controlled; the choice of chat model
affects absolute quality but not the relative ranking. Embeddings use OpenAI
\texttt{text-embedding-3-small} (the client separates the chat and embedding
providers, since the chat provider exposes no embedding endpoint). Quality
judges use a different model family (\texttt{gpt-4.1}) to avoid self-evaluation
bias. Chinese-language simulations are run natively; human-facing deliverables
(screenplays, quoted memories, figure labels) are translated to English for
presentation, with source-language originals retained (Appendix~\todo{ref}
gives representative Chinese/English pairs). \todo{finalize model versions and
per-run token/cost numbers}
...
\section{Source-language originals}
\label{app:examples}
The paper is in English; simulations run natively in the source language.
To document that our results are produced on the source text (not on a
translated proxy), this appendix pairs representative outputs with their
Chinese originals. Translations are produced in a single pass by the same
model family; originals are the verbatim simulation records.

\subsection{Representative consensus memories}
\label{app:consensus}
Multi-owner entries produced by the consensus merge, with owners
(Table~\ref{tab:app-mem}). These are the atomic memories jointly held after
several characters record the same event.
\begin{table}[h]
\centering\small
\begin{tabular}{p{4.2cm}p{4.6cm}p{3.2cm}}
\toprule
Source (\begin{CJK}{UTF8}{gbsn}中文原文\end{CJK}) & English translation & Owners \\
\midrule
\begin{CJK}{UTF8}{gbsn}刘备、关羽、张飞桃园结义\end{CJK} &
Liu Bei, Guan Yu, and Zhang Fei swear brotherhood in the peach garden &
liubei, guanyu, zhangfei \\
\todo{populate from the run's \texttt{ltm\_final.json} via
\texttt{society.translate}; keep 5--8 rows} & & \\
\bottomrule
\end{tabular}
\caption{Consensus memories: Chinese source, English translation, owner set.}
\label{tab:app-mem}
\end{table}

\subsection{Screenplay excerpt (bilingual)}
\label{app:screenplay}
\todo{A short scene from the Three Kingdoms sequel, source \texttt{screenplay.md}
(zh) beside \texttt{screenplay\_en.md} (en), rendered from the same event log.}

\subsection{Bootstrapped character goals}
\label{app:goals}
\todo{2--3 characters' self-derived goals (e.g., Cao Cao: ``march on Xuzhou to
avenge my father'') with the Chinese \texttt{conclude}/\texttt{push\_goal}
originals, showing goals emerge from recalled memory.}
